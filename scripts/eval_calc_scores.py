...
p = {
    ...
    'error_tpath':
        os.path.join('{eval_path}', '{error_dir_path}',
                     'errors_{scene_id:06d}.json'),

    # Template of path to the output file with established matches and
    # calculated scores.
    'out_matches_tpath':
        os.path.join('{eval_path}', '{error_dir_path}',
...
for err_type in p['correct_th']:
    parser.add_argument('--correct_th_' + err_type,
...
parser.add_argument('--error_dir_paths',
...
for error_dir_path in p['error_dir_paths']:
    ...
    for scene_id, scene_targets in targets_org.items():
        ...
        for im_id, im_targets in scene_targets.items():
            ...
            if p['visib_gt_min'] >= 0:
                # All GT poses visible from at least 100 * p['visib_gt_min']
                # percent are considered valid.
                ...
                # k most visible GT poses are considered valid, where k is given
                # by the "inst_count" item loaded from "targets_filename".
                gt_ids_sorted = sorted(
                    range(len(im_gt)),
                    key=lambda gt_id: im_gt_info[gt_id]['visib_fract'],
                    reverse=True)
                to_add = {
                    obj_id: trg['inst_count']
                    for obj_id, trg in im_targets.items()
                }
        ...
        scene_errs_path = p['error_tpath'].format(eval_path=p['eval_path'],
        ...
        matches += pose_matching.match_poses_scene(scene_id, scene_gt_curr,
                                                   scene_gt_valid, scene_errs,
                                                   p['correct_th'][err_type],
                                                   n_top)
    ...
    scores = score.calc_localization_scores(dp_split['scene_ids'],
                                            dp_model['obj_ids'], matches, n_top)

    # Save scores.
    scores_path = p['out_scores_tpath'].format(eval_path=p['eval_path'],
                                               error_dir_path=error_dir_path,
                                               score_sign=score_sign)
    inout.save_json(scores_path, scores)

    # Save matches.
    matches_path = p['out_matches_tpath'].format(eval_path=p['eval_path'],
                                                 error_dir_path=error_dir_path,
                                                 score_sign=score_sign)
